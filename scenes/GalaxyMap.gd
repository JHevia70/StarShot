...
func _build_galaxy() -> void:
	...
	while remaining > 0:
		...
		remaining -= chunk
	# Estrellas dispersas
	var min_r: float = 22.0; var max_r: float = 64.0; var min_sep: float = 8.0; var height_jitter: float = 6.0
...
func _spawn_star(pos: Vector3, stype: String, idx: int) -> Node3D:
	var n: Node3D = Node3D.new(); n.name = "Star_%d" % idx; n.position = pos
	var mi: MeshInstance3D = MeshInstance3D.new(); var sm: SphereMesh = SphereMesh.new()
	sm.radius = 1.5; sm.height = 3.0; sm.radial_segments = 64; sm.rings = 32; mi.mesh = sm
	var mat: StandardMaterial3D = StandardMaterial3D.new(); var col: Color = _star_color(stype)
	mat.shading_mode = BaseMaterial3D.SHADING_MODE_UNSHADED; mat.albedo_color = col; mat.emission_enabled = true; mat.emission = col; mat.emission_energy_multiplier = 3.0
	mi.material_override = mat; n.add_child(mi)
	var lbl: Label3D = Label3D.new(); lbl.text = "Estrella %d (%s)" % [idx + 1, stype]; lbl.billboard = BaseMaterial3D.BILLBOARD_ENABLED; lbl.position = Vector3(0, 2.0, 0); n.add_child(lbl)
	_add_corona(n, col)
	return n
...
func _add_corona(star: Node3D, col: Color) -> void:
	...
	var grad: Gradient = Gradient.new(); grad.colors = PackedColorArray([Color(col.r, col.g, col.b, 0.9), Color(col.r, col.g, col.b, 0.0)])
	var ramp: GradientTexture1D = GradientTexture1D.new(); ramp.gradient = grad; pm.color_ramp = ramp
	particles.process_material = pm
	var quad: QuadMesh = QuadMesh.new(); quad.size = Vector2(0.6, 0.6); particles.draw_pass_1 = quad
	star.add_child(particles)

func _spawn_planet(star_node: Node3D, orbit_radius: float, star_index: int, local_index: int) -> Node3D:
	var p: Node3D = Node3D.new(); p.name = "Planet_%d_%d" % [star_index, local_index]; star_node.add_child(p)
	# render
	var mi: MeshInstance3D = MeshInstance3D.new(); var sm: SphereMesh = SphereMesh.new()
	sm.radius = 0.45 + 0.2 * randf(); sm.height = sm.radius * 2.0; sm.radial_segments = 32; sm.rings = 16; mi.mesh = sm
	var m: StandardMaterial3D = StandardMaterial3D.new(); m.albedo_color = Color.from_hsv(randf(), 0.55, 0.95); mi.material_override = m; p.add_child(mi)
	# spin
	var spin_s: Script = load("res://scenes/Spin.gd") as Script
	if spin_s != null:
		var spin: Node = spin_s.new(); spin.set("speed", 0.06); p.add_child(spin)
	# label
	var lbl: Label3D = Label3D.new(); lbl.text = "P%d-%d" % [star_index + 1, local_index + 1]; lbl.billboard = BaseMaterial3D.BILLBOARD_ENABLED; lbl.position = Vector3(0, sm.radius + 0.6, 0); p.add_child(lbl)
	# órbita
	var orbit_s: Script = load("res://scenes/Orbit.gd") as Script
	if orbit_s != null:
		var orbit: Node = orbit_s.new(); p.add_child(orbit); orbit.set("orbit_radius", orbit_radius); orbit.set("orbit_speed", 0.22 + randf() * 0.22)
	# click
	var area: Area3D = Area3D.new(); area.input_ray_pickable = true
	var colshape: CollisionShape3D = CollisionShape3D.new(); var sph: SphereShape3D = SphereShape3D.new(); sph.radius = sm.radius * 1.25; colshape.shape = sph; area.add_child(colshape); p.add_child(area)
...
func _on_planet_input(_camera: Node, event: InputEvent, _click_position: Vector3, _click_normal: Vector3, _shape_idx: int, planet: Node) -> void:
	if event is InputEventMouseButton:
		...
		if mb.button_index == MOUSE_BUTTON_LEFT and mb.pressed:
			...
			if allowed and not _transitioning:
				_transitioning = true
				call_deferred("_goto_transmission", planet)

func _goto_transmission(planet: Node) -> void:
	...
	if ResourceLoader.exists(scene_path):
		...
		if err != OK:
			_notify("No se pudo cargar Transmission (código %d)" % err)
	else:
		_notify("No se encontró Transmission.tscn")
	_transitioning = false

func _notify(msg: String) -> void:
	var d: AcceptDialog = AcceptDialog.new()
	d.title = "Aviso"; d.dialog_text = msg; add_child(d); d.popup_centered()
...
func _update_camera() -> void:
	var rot: Basis = Basis(); rot = rot.rotated(Vector3.UP, yaw); rot = rot.rotated(Vector3.RIGHT, pitch)
...
func _on_hangar() -> void:
	...
	if ResourceLoader.exists(shop_path):
		...
		if s != null:
			var inst: Node = s.instantiate(); get_tree().current_scene.add_child(inst)
	else:

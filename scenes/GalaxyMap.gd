...
@export var sky_path: String = "res://textures/nebula_panorama.hdr" # si no existe, usa cielo procedural
...
@onready var info_label: Label = $HUD/HBox/Info as Label
...
func _ready() -> void:
	...
	# Luz básica para que las mallas estándar no se vean negras
	sun.light_energy = 1.2
	# Progreso: 1 planeta al inicio (solo el 0)
	highest_unlocked = -1
...
func _build_galaxy() -> void:
	...
	# Reparto 5..10 planetas por estrella hasta 100
...
func _spawn_planet(star_node: Node3D, orbit_radius: float, star_index: int, local_index: int) -> Node3D:
	...
	# visibilidad: ganados (<= highest) + siguiente (== next_visible)
	var gidx: int = planets.size()
	var unlocked: bool = (gidx <= highest_unlocked) or (gidx == next_visible)
	mi.visible = unlocked
